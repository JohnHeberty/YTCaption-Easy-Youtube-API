...
- **Face blend improvement (v23.1):**
  - Protected region reduced from full head+hair+neck to inner face only (~23% of previous mask)
  - Jaw, chin, neck and hair edges are now generated by SE8, avoiding the "cutout" collage look
  - Feather narrowed from 21px to 11px Gaussian for sharper, more natural transition
  - Color harmonization (Reinhard LAB) applied before final face re-blend
  - **E2E validated:** job `cr_75c5996737ab` completed; face_protect_mask = 29.9k px vs head_adjusted = 131.1k px
  - **Resultado insuficiente visualmente** → plano master criado em `/root/YTCaption-Easy-Youtube-API/PLAN-HEAD.md`
