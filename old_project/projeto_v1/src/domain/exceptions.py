...
class UnsupportedFormatError(VideoUploadError):
    ...
    def __init__(self, format_: str, supported_formats: list):
        self.format = format_
        self.supported_formats = supported_formats
        super().__init__(
            f"Format {format_} not supported. "
            f"Supported: {', '.join(supported_formats[:10])}"
        )
...
class InvalidVideoFileError(VideoUploadError):
    ...
    def __init__(self, reason: str):
        self.reason = reason
        super().__init__(f"Invalid video file: {reason}")
