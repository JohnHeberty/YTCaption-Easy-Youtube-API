...
def match_lora(lora_data: dict[str, Any], to_load: dict[str, str]) -> tuple[dict, dict]:
    ...
    for model_key, lora_key in to_load.items():
        ...
        alpha, alpha_name = _extract_alpha(model_key, lora_data)
        if alpha_name is not None:
            loaded_keys.add(alpha_name)

        matched = False
        for matcher in (_match_regular_lora, _match_loha, _match_lokr, _match_glora, _match_diff):
            result = matcher(model_key, lora_data, to_load, alpha, loaded_keys)
            if result is not None:
                patch_dict[result[0]] = result[1]
                loaded_keys.update(result[2])
                matched = True
                break

        if not matched:
            _match_w_norm(model_key, lora_data, to_load, loaded_keys, patch_dict)

    remaining_dict = {k: v for k, v in lora_data.items() if k not in loaded_keys}
    return patch_dict, remaining_dict


def _extract_alpha(model_key: str, lora_data: dict[str, Any]) -> tuple[Any, str | None]:
    """Extract alpha value for a model key."""
    alpha_name = f"{model_key}.alpha"
    if alpha_name in lora_data:
        return lora_data[alpha_name].item(), alpha_name
    return None, None


def _match_regular_lora(
    model_key: str, lora_data: dict, to_load: dict, alpha: Any, loaded_keys: set
) -> tuple[str, tuple, set] | None:
    """Match regular/diffusers/transformers LoRA keys."""
    ...
    patch_dict_keys = {A_name, B_name}
    if mid is not None:
        patch_dict_keys.add(mid_name)

    return (to_load[model_key], ("lora", (lora_data[A_name], lora_data[B_name], alpha, mid)), patch_dict_keys)
...
def _match_loha(
    ...
    hada_t1 = None
    hada_t2 = None
    patch_keys = {hada_w1_a, hada_w1_b, hada_w2_a, hada_w2_b}
    if hada_t1_name in lora_data:
        hada_t1 = lora_data[hada_t1_name]
        hada_t2 = lora_data[hada_t2_name]
        patch_keys.update({hada_t1_name, hada_t2_name})

    return (to_load[model_key], ("loha", (
...
def _match_lokr(
    ...
    if not any(v is not None for v in [lokr_w1, lokr_w2, lokr_w1_a, lokr_w2_a]):
        return None

    return (to_load[model_key], ("lokr", (
...
def _match_glora(
    ...
    if a1_name not in lora_data:
        return None

    return (to_load[model_key], ("glora", (
...
def _match_diff(
    ...
    """Match diff/w_norm keys."""
    diff_name = f"{model_key}.diff"
    diff_weight = lora_data.get(diff_name)
    if diff_weight is None:
        return None

    patch_keys = {diff_name}
    diff_bias_name = f"{model_key}.diff_b"
    diff_bias = lora_data.get(diff_bias_name)
    if diff_bias is not None:
        patch_keys.add(diff_bias_name)
        bias_key = f"{model_key[:-len('.weight')]}.bias" if model_key.endswith('.weight') else f"{model_key}.bias"
        return (to_load[model_key], ("diff", (diff_weight,)), patch_keys)

    return (to_load[model_key], ("diff", (diff_weight,)), patch_keys)


def _match_w_norm(
    model_key: str, lora_data: dict, to_load: dict, loaded_keys: set, patch_dict: dict
) -> None:
    """Match w_norm/b_norm keys (adds to patch_dict directly)."""
