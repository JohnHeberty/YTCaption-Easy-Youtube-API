...
class Pipeline:
    ...
    def refresh_base_model(self, name: str, vae_name: Optional[str] = None):
        ...
        vae_filename = None
        if vae_name is not None and vae_name not in ('None', 'none', ''):
            ...
            try:
                ...
                if not isinstance(self.model_base.unet_with_lora.model, SDXL):
                    raise NotImplementedError(
                        "Only SDXL base models are supported in SE9"
                    )
            except ImportError:
    ...
    def process_diffusion(
        ...
        if refiner_swap_method == 'joint':
            ...
            logger.info("SE9 VAE-based swap.")
            ...
            try:
                ...
                noise_mean = torch.mean(
                    modules.patch.patch_settings[os.getpid()].eps_record,
                    dim=1, keepdim=True,
                )
            except (ImportError, AttributeError):
                noise_mean = None
