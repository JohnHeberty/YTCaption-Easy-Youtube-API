...
class TextRegionExtractor:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        self.reader = get_ocr_detector()
        
        logger.info(f"TextRegionExtractor initialized: downscale={self.target_width}px, engine=PaddleOCR")
    ...
    def _detect_words(self, roi_prep: np.ndarray) -> List[dict]:
        ...
        """
        try:
            # Multi-Engine OCR retorna List[OCRResult]
            ocr_results = self.reader.detect_text(roi_prep)
            
            # Converter OCRResult para formato de palavras
            words = []
            ...
            for result in ocr_results:
                ...
                # result.bbox format: Tuple[int, int, int, int] = (x, y, w, h)
                x, y, w, h = result.bbox
                
                words.append({
                    'text': text,
                    'bbox': (x, y, w, h),
                    'conf': result.confidence  # Já normalizado [0-1]
                })
