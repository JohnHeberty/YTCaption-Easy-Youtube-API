...
**Data**: 07/02/2026  
**Data Correção**: 11/02/2026  
**Job ID Afetado**: `2JSmLM9TPL9Y6qDeUQr5ND`  
**Severidade**: 🟢 **RESOLVIDO** - Correções implementadas, aguardando teste
...
- [x] **CRÍTICO**: Corrigir `trim_video()` - trocar stream copy por re-encode
  - **Implementado**: Agora usa `-c:v libx264 -c:a aac` ao invés de `-c copy`
  - **Arquivo**: `app/video_builder.py` linha ~370-385
- [x] **CRÍTICO**: Investigar bug na concatenação - adicionar logs e validações
  - **Implementado**: Adicionado cálculo de expected_duration e validação pós-concatenação
  - **Arquivo**: `app/video_builder.py` linha ~115-200
- [x] **IMPORTANTE**: Garantir único stream de áudio em `burn_subtitles()`
  - **Implementado**: Adicionado `-map 0:v:0 -map 0:a:0` 
  - **Arquivo**: `app/video_builder.py` linha ~303-304
- [x] **IMPORTANTE**: Adicionar validação final de duração
  - **Implementado**: Tolerância de 2s e log de warning
  - **Arquivo**: `app/video_builder.py` linha ~185-200
...
**Última atualização**: 11/02/2026 12:00  
...
1. **trim_video()**: Alterado de stream copy para re-encode (libx264)
2. **burn_subtitles()**: Adicionado -map para único stream de áudio/vídeo  
3. **concatenate_videos()**: Adicionado validação de duração pós-concatenação
4. **OCR GPU Toggle**: Adicionado suporte a GPU via variável OCR_USE_GPU
