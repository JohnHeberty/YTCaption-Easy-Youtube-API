...
async def run_nsfw(job: ClothesRemovalJob, store: ClothesRemovalJobStore) -> None:
    ...
    try:
        ...
        # (jaw, cheeks, temples) between the central protected face and the body.
        # The central face is still protected by face_protect_mask during composite.
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # ─── Face preservation with distance-transform feather ───
            # Protect only the CENTRAL FACE (face_protect_mask). Forehead, cheeks,
            # jaw, chin, neck and hair are generated by the model. The transition
            # alpha is computed from the Euclidean distance to the mask edge: 1.0
            # at the center, fading to 0.0 over a fixed band (~40px). This gives
            # the model a clear transition band to paint natural skin, avoiding
            # both hard edges and the "airbrushed" look of a large Gaussian blur.
            ...
            face_f = _np.clip(face_dist / transition_width, 0, 1)

            # First paste: central face preserved, outer band blended
