...
class PipelineOrchestrator:
    ...
    Pipeline:
    1. Download vídeo (se2-video-downloader)
    2. Normalização áudio (se3-audio-normalization)
    3. Transcrição (se4-audio-transcriber)

    Args:
        video_client: Cliente do serviço se2-video-downloader
        audio_client: Cliente do serviço se3-audio-normalization
        transcription_client: Cliente do serviço se4-audio-transcriber
    ...
    async def _execute_download(
        ...
        try:
            ...
            if not stage.job_id:
                raise RuntimeError(f"se2-video-downloader não retornou job_id: {resp}")
            ...
            status = await self._wait_until_done(
                self._video_client, stage.job_id, stage, "se2-video-downloader"
            )
            if not status:
    ...
    async def _execute_normalization(
        ...
        try:
            cfg = get_microservice_config("se3-audio-normalization")
            ...
            if not stage.job_id:
                raise RuntimeError(f"se3-audio-normalization não retornou job_id: {resp}")

            logger.info(f"Audio normalization job submitted: {stage.job_id}")

            status = await self._wait_until_done(
                self._audio_client, stage.job_id, stage, "se3-audio-normalization"
            )
            if not status:
    ...
    async def _execute_transcription(
        ...
        try:
            cfg = get_microservice_config("se4-audio-transcriber")
            defaults = (cfg.get("default_params") or {}).copy()
            ...
            if not stage.job_id:
                raise RuntimeError(f"se4-audio-transcriber não retornou job_id: {resp}")

            logger.info(f"Transcription job submitted: {stage.job_id}")

            status = await self._wait_until_done(
                self._transcription_client, stage.job_id, stage, "se4-audio-transcriber"
            )
            if not status:
