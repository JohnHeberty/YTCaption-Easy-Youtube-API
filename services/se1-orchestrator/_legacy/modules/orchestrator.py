...
class PipelineOrchestrator:
    ...
    def __init__(self, redis_store=None):
        ...
        self.video_client = MicroserviceClient("se2-video-downloader")
        self.audio_client = MicroserviceClient("se3-audio-normalization")
        self.transcription_client = MicroserviceClient("se4-audio-transcriber")
    ...
    async def check_services_health(self) -> Dict[str, str]:
        results = {}
        for name, client in [
            ("se2-video-downloader", self.video_client),
            ("se3-audio-normalization", self.audio_client),
            ("se4-audio-transcriber", self.transcription_client),
        ]:
    ...
    async def _execute_download(self, job: PipelineJob) -> Optional[tuple[bytes, str]]:
        ...
        try:
            ...
            if not stage.job_id:
                raise RuntimeError(f"se2-video-downloader não retornou job_id válido: {resp}")
