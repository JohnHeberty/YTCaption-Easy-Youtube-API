...
class TestPipelineOrchestrator:
    ...
    async def test_check_services_health(self, orchestrator):
        ...
        orchestrator._health_checker.check_all.return_value = {
            "se2-video-downloader": "healthy",
            "se3-audio-normalization": "healthy",
            "se4-audio-transcriber": "healthy",
        }

        result = await orchestrator.check_services_health()

        assert result["se2-video-downloader"] == "healthy"
        assert result["se3-audio-normalization"] == "healthy"
        assert result["se4-audio-transcriber"] == "healthy"
