...
def get_health_checker() -> HealthChecker:
    ...
    """
    clients = {
        "se2-video-downloader": get_microservice_client("se2-video-downloader"),
        "se3-audio-normalization": get_microservice_client("se3-audio-normalization"),
        "se4-audio-transcriber": get_microservice_client("se4-audio-transcriber"),
    }
...
def get_pipeline_orchestrator(redis_store: Optional[RedisStore] = None) -> PipelineOrchestrator:
    ...
    video_client = get_microservice_client("se2-video-downloader")
    audio_client = get_microservice_client("se3-audio-normalization")
    transcription_client = get_microservice_client("se4-audio-transcriber")
...
def get_orchestrator_with_custom_clients(
    ...
    video_client = video_client or get_microservice_client("se2-video-downloader")
    audio_client = audio_client or get_microservice_client("se3-audio-normalization")
    transcription_client = transcription_client or get_microservice_client("se4-audio-transcriber")

    # Cria HealthChecker com clients customizados
    clients = {
        "se2-video-downloader": video_client,
        "se3-audio-normalization": audio_client,
        "se4-audio-transcriber": transcription_client,
    }
