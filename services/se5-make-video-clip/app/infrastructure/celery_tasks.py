...
def process_download_pipeline(self, job_id: str):
    ...
    Etapas:
    1. Buscar shorts via youtube-search API
    2. Baixar cada short via video-downloader API
    3. Validar cada video (transform, crop, OCR)
    4. Mover aprovados para data/approved/videos/
    """
...
async def _process_download_pipeline_async(job_id: str):
    ...
    await _save_checkpoint(job_id, "searching_shorts_completed")
    ...
    # Etapa 3: Validar cada video (transform, crop, OCR)
    ...
    for i, video_id in enumerate(downloaded_ids):
        ...
        final_path = await _transform_crop_and_validate_video(
            video_id=video_id,
